...
struct threshold_fixture {
   ...
   bool test_threshold_common_(std::map<std::filesystem::path, uintmax_t>& available, std::map<std::filesystem::path, int>& dev, uint32_t warning_threshold=75)
   {
      ...
      mock_get_space = [available]( const std::filesystem::path& p, boost::system::error_code& ec) mutable -> std::filesystem::space_info {
...
BOOST_AUTO_TEST_SUITE(threshold_tests)
   ...
   {
      mock_get_space = [ i = 0 ]( const std::filesystem::path& p, boost::system::error_code& ec) mutable -> std::filesystem::space_info {
         if ( i == 3 ) {
