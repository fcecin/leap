...
struct add_file_system_fixture {
   struct mock_space_provider {
      ...
      std::filesystem::space_info get_space(const std::filesystem::path& p, boost::system::error_code& ec) const {
   ...
   void test_add_file_systems_common(std::vector<int>& capacity, std::vector<int>& available, std::vector<int>& devs) {
      mock_get_space = [ i = 0, capacity, available ]( const std::filesystem::path& p, boost::system::error_code& ec) mutable -> std::filesystem::space_info {
   ...
   std::function<std::filesystem::space_info(const std::filesystem::path& p, boost::system::error_code& ec)> mock_get_space;
...
BOOST_AUTO_TEST_SUITE(space_handler_tests)
   ...
   {
      mock_get_space = []( const std::filesystem::path& p, boost::system::error_code& ec) -> std::filesystem::space_info {
